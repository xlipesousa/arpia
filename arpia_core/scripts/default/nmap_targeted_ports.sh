...
# Nmap targeted scan leveraging ports discovered during reconnaissance.
# Macros disponíveis:
...
if ! has_non_empty "${TARGETS_WITH_PORTS}"; then
	if ! has_non_empty "${AGGREGATED_PORTS}"; then
		echo "[WARN] Nenhuma porta detectada para ${PROJECT_NAME}." >&2
		exit 0
	fi
fi
...
run_scan() {
	...
	echo "[INFO] Nmap targeted scan para ${target} (${ports})"
	"${NMAP_BIN}" -Pn -sS -sV --reason -p "${ports}" "$target" \
		-oA "${OUTPUT_DIR}/nmap_targeted_${safe_target}" || true
	printf '\n'
}
...
if has_non_empty "${TARGETS_WITH_PORTS}"; then
	...
	echo "[INFO] Utilizando portas agregadas para todos os hosts." >&2
	...
	while IFS= read -r host; do
		host=$(trim "${host}")
		[[ -z "${host}" ]] && continue
		run_scan "${host}" "${AGGREGATED_PORTS}"
	done <<< "${TARGET_HOSTS}"
fi
